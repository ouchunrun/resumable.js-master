...
module.exports = resumable = function(temporaryFolder){
  ...
  try {
    mkdirSyncRecursive($.temporaryFolder)
  ...
  $.get = function(req, callback){
    ...
    if(validateRequest(chunkNumber, chunkSize, totalSize, identifier, filename)=='valid') {
      ...
      console.log("chunkFilename11: ", chunkFilename)
  ...
  $.post = function (req, callback) {
    ...
    if (validation === 'valid') {  // 有效
      var chunkFilename = getChunkFilename(chunkNumber, identifier);
      console.log("chunkFilename22: ", chunkFilename)
      console.warn("files[$.fileParameterName].path: ", files[$.fileParameterName].path)

      console.warn("$.fileParameterName: ", $.fileParameterName)
      // 保存块 (TODO: OVERWRITE)
      fs.rename(files[$.fileParameterName].path, chunkFilename, function () {
        ...
        var testChunkExists = function () {
          ...
          console.log("chunkFilename333: ", chunkFilename)

          fs.exists(chunkFilename, function (exists) {
            console.log("exists: ", exists)
            console.warn("currentTestChunk: ",  currentTestChunk)
            console.warn("numberOfChunks: ", numberOfChunks)
            if (exists) {
              currentTestChunk++;
              if (currentTestChunk > numberOfChunks) {
                callback('完成', filename, original_filename, identifier);
              } else {
                // Recursion
                testChunkExists();
              }
            } else {
              callback('部分完成', filename, original_filename, identifier);
            }
          });
        }
        testChunkExists();
  ...
  $.write = function(identifier, writableStream, options) {
      ...
      var pipeChunk = function(number) {
          ...
          console.log("chunkFilename44: ", chunkFilename)
          fs.exists(chunkFilename, function(exists) {
